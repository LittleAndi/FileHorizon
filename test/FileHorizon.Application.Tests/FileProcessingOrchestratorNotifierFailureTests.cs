...
public class FileProcessingOrchestratorNotifierFailureTests
{
    ...
    [Fact]
    public async Task FailurePublish_MissingReader_NotifiesFailure()
    {
        var srcFile = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N") + ".dat");
        ...
        var ev = new FileEvent(
            ...
            Protocol: "mystery", // will not match local or sftp readers
        ...
        var readers = new List<IFileContentReader> { new Infrastructure.Processing.LocalFileContentReader(NullLogger<Infrastructure.Processing.LocalFileContentReader>.Instance) }; // only local provided
        ...
        var destinations = new DestinationsOptions { Local = [new LocalDestinationOptions { Name = "OutboxA", RootPath = Path.Combine(Path.GetTempPath(), "orchestrator-failure-tests") }] };
        ...
        Assert.Equal(ProcessingStatus.Failure, notifier.Published[0].Status);
        // Cleanup
